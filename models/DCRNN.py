...
class LayerParams:
    def __init__(self, rnn_network: torch.nn.Module, layer_type: str) -> None:
...
class DCGRUCell(torch.nn.Module):
    def __init__(self, num_units, adj_mx, max_diffusion_step, num_nodes, nonlinearity='tanh', filter_type='laplacian', use_gc_for_ru=True) -> None:
        super().__init__()
        """
        :param num_units:
        :param adj_mx:
        ...
        if filter_type == 'laplacian':
            supports.append(calculate_scaled_laplacian(
    ...
    def forward(self, inputs, hx):
        ...
        :return 
        - Output: A `2-D` tesnor with shape `(B, num_nodes * rnn_units)`.
        """
        ...
        r, u = torch.split(
            tesnor=value, split_size_or_sections=self._num_units, dim=-1)
